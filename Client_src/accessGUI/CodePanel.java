	...
	private void checkCode() {
		try {
			if(!codeF.getText().equals("")) {
				if(ClientRdF.checkCode(ClientRdF.email, codeF.getText())) {
					JPanel p = new JPanel();
				    ...
				    if(value == JOptionPane.OK_OPTION) {
				    	if(ClientRdF.updateCode(String.valueOf(pf.getPassword()), codeF.getText())) {
				    	...
				    	JOptionPane.showMessageDialog(null, "Password Non Inserita" + "\nBenvenuto! La tua password corrisponde al codice di attivazione, consigliamo di modificarla",
								"Registrazione Completata", JOptionPane.INFORMATION_MESSAGE);
						ClientRdF.updateCode(codeF.getText(), codeF.getText());
						codeFrame.dispose();
						ClientRdF.setOnline(true);
						new PlayerFrame();
				    }
